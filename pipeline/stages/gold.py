...
from pipeline.stages.common import get_silver_path
...
def generate_top_10_fastest_vehicles():
    ...
    df = (
        df.withColumn("datetime", F.from_unixtime(F.col("timestamp") / 1000))
...
def gold_top_fastest_op(data_lake_path: str = "data_lake"):
    ...
    df = spark.read.parquet(silver_path)
    df = df.withColumn(
        "date_hour", F.date_format(F.col("timestamp") / 1000, "yyyy-MM-dd-HH")
    )
    ...
    top_vehicles.write.mode("overwrite").parquet(
        os.path.join(gold_path, "top_10_fastest")
    )
